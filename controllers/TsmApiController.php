...
class TsmApiController extends Controller
{
	...
	if ($scantime == 0){
        ...
        #$qry = "select ip.* from item_prices ip where ip.realm_id=:realm and ip.faction_id=:faction and `datetime`=(select max(datetime) from item_prices where realm_id=:realm and faction_id=:faction) and faction_id=:faction group by ip.itemid";
